...
class Application {
  ...
  async deploy(driver) {
    ...
    if (!instance.deployCompute || typeof instance.deployCompute !== 'function') {
      throw new Error('Driver does not have a `deployCompute` function');
    }
    if (!instance.deployStorage || typeof instance.deployStorage !== 'function') {
      throw new Error('Driver does not have a `deployStorage` function');
    }
    ...
    for (const resource of this._resources.storage) {
      instance.deployStorage(resource._config);
    }

    // Deploy compute instances
    for (const resource of this._resources.compute) {
      instance.deployCompute(resource._config);
    }
