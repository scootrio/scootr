...
function application(name) {
  ...
  const builders = self => ({
    ...
    with: resource => {
      ...
      // Safety check to ensure that there are no duplicate resource IDs
      if (ids.includes(resource.config.id)) {
  ...
  const behaviors = self => ({
    ...
    deploy: async (driver, region) => {
      ...
      // First, deploy all of our triggers (these are the events in the system)
      ...
      // Deploy storage used by compute instances
      for (const resource of resources.storage) {
        instance.onStorage(resource.config);
      }

      // Finally deploy the direct connections between resources and topics. We first hand the triggers to the driver,
      // which are the connections from events to compute resources. We then hand the references from compute resources
      // to other resources.
      for (const resource of resources.triggers) {
        instance.onTrigger(resource.config);
      }
      for (const resource of resources.references) {
        instance.onReference(resource.config);
      }
