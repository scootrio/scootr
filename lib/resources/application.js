  ...
  const behaviors = self => ({
    ...
    deploy: async (driver, region) => {
      ...
      for (const resource of resources.references) {
        instance.onReference(
          Object.assign(resource.config, {
            source: resource.config.source.config,
            target: resource.config.target.config
          })
        );
      }
      ...
      try {
        ...
        // Currently this is handled by the caller. Later on, once we standardize a format, we can move this into
        // the runtime library itself to provide an additonal layer of abstraction and further increase the usability
      } catch (err) {
